...
"order by seller desc"
    ...
    public int getBestSellerListCount() throws SQLException {
        List<Product> list = new ArrayList<>();
        Connection con = null;
        PreparedStatement pstm = null;
        ResultSet rs = null;
        ...
        try {
            con = DBHelper.makeConnection();
            if (con != null) {
                String sql = "select count([OrderDetail].productID)" +
"from [Product]  join [OrderDetail] " +
"on [Product].productID = [OrderDetail].productID where status like 'av' " +
"and shopID in (" +
"select shopID from shop where accountID in ( " +
        ...
        try {
            ...
            if (con != null) {
                String sql = " select * from product,Shop,account "
                        + "where Shop.shopID = Product.shopID and status like 'av' and isDeleted = 0 "
                        + "and account.accountID =shop.accountID order by productID desc";
                pstm = con.prepareStatement(sql);
